...
static void McpsConfirm( McpsConfirm_t *mcpsConfirm )
{
    ...
    if( AppData.BufferSize != 0 )
    {
        ...
        {
            DebugPrintf( "CONFIRMED - %s\r\n", ( mcpsConfirm->AckReceived != 0 ) ? "ACK" : "NACK" );
        }
        else
        {
            DebugPrintf( "UNCONFIRMED\r\n" );
        }
        PrintHexBuffer( AppData.Buffer, AppData.BufferSize );
    }

    DebugPrintf( "\r\n" );
    DebugPrintf( "DATA RATE   : DR_%d\r\n", mcpsConfirm->Datarate );

    mibGet.Type  = MIB_CHANNELS;
    if( LoRaMacMibGetRequestConfirm( &mibGet ) == LORAMAC_STATUS_OK )
    {
        DebugPrintf( "U/L FREQ    : %lu\r\n", mibGet.Param.ChannelList[mcpsConfirm->Channel].Frequency );
    }

    DebugPrintf( "TX POWER    : %d\r\n", mcpsConfirm->TxPower );

    mibGet.Type  = MIB_CHANNELS_MASK;
    if( LoRaMacMibGetRequestConfirm( &mibGet ) == LORAMAC_STATUS_OK )
    {
        DebugPrintf("CHANNEL MASK: ");
...
#endif
        {
            DebugPrintf("%04X ", mibGet.Param.ChannelsMask[i] );
        }
        DebugPrintf("\r\n");
    }
...
static void McpsIndication( McpsIndication_t *mcpsIndication )
{
    ...
    if( mcpsIndication->RxData == true )
    {
		...
		DebugPrintf("Turning led on? %d\n", ( ( AppLedStateOn & 0x01 ) != 0 ) ? 1 : 0);
            }
            break;
        case 224:
    ...
    DebugPrintf( "RX WINDOW   : %s\r\n", slotStrings[mcpsIndication->RxSlot] );
    DebugPrintf( "RX PORT     : %d\r\n", mcpsIndication->Port );
    ...
    DebugPrintf( "RX RSSI     : %d\r\n", mcpsIndication->Rssi );
    DebugPrintf( "RX SNR      : %d\r\n", mcpsIndication->Snr );
...
int main( void )
{
    ...
    DebugPrintf( "###### ===== ClassA demo application v1.0.RC1 ==== ######\r\n\r\n" );
    GpioWrite( &Led1, 0 );
    DebugPrintf("Initialising - Turning LED off\r\n");
    ...
    while( 1 )
    {
        ...
        switch( DeviceState )
        {
            ...
            case DEVICE_STATE_JOIN:
            {
                mibReq.Type = MIB_DEV_EUI;
                LoRaMacMibGetRequestConfirm( &mibReq );
                DebugPrintf( "DevEui      : %02X", mibReq.Param.DevEui[0] );
                for( int i = 1; i < 8; i++ )
                {
                    DebugPrintf( "-%02X", mibReq.Param.DevEui[i] );
                }
                DebugPrintf( "\r\n" );
                mibReq.Type = MIB_JOIN_EUI;
                LoRaMacMibGetRequestConfirm( &mibReq );
                DebugPrintf( "AppEui      : %02X", mibReq.Param.JoinEui[0] );
                for( int i = 1; i < 8; i++ )
                {
                    DebugPrintf( "-%02X", mibReq.Param.JoinEui[i] );
                }
                DebugPrintf( "\r\n" );
                DebugPrintf( "AppKey      : %02X", NwkKey[0] );
                for( int i = 1; i < 16; i++ )
                {
                    DebugPrintf( " %02X", NwkKey[i] );
                }
                DebugPrintf( "\n\r\n" );
#if( OVER_THE_AIR_ACTIVATION == 0 )
                DebugPrintf( "###### ===== JOINED ==== ######\r\n" );
                DebugPrintf( "\r\nABP\r\n\r\n" );
                DebugPrintf( "DevAddr     : %08lX\r\n", DevAddr );
                DebugPrintf( "NwkSKey     : %02X", FNwkSIntKey[0] );
                for( int i = 1; i < 16; i++ )
                {
                    DebugPrintf( " %02X", FNwkSIntKey[i] );
                }
                DebugPrintf( "\r\n" );
                DebugPrintf( "AppSKey     : %02X", AppSKey[0] );
                for( int i = 1; i < 16; i++ )
                {
                    DebugPrintf( " %02X", AppSKey[i] );
                }
                DebugPrintf( "\n\r\n" );
            ...
            {
                DebugPrintf("DEVICE_STATE_CYCLE\r\n");
                DeviceState = DEVICE_STATE_SLEEP;
                if( ComplianceTest.Running == true )
                {
                ...
                DebugPrintf("Scheduling next tx in %dms\r\n", TxDutyCycleTime);
                DebugPrintf("HAL Ticks %d\r\n", HAL_GetTick());
		...
		//DebugPrintf("DEVICE_STATE_SLEEP\r\n");
                ...
                {
		    DebugPrintf("Not sleeping, still processing to complete\r\n");
		    ...
		    //DelayMs(250); // Test
                    ...
                    //RtcSetAlarm(10); // added to make it sleep a while
